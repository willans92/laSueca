var url = '../Controlador/ReporteTraspasoProducto_Controlador.php';
...
function reporteDetallado() {
    ...
    $.get(url, {proceso: 'traspasoDetallado', de: de, hasta: hasta}, function (response) {
        ...
        if (json.error.length > 0) {
            ...
            html += "<th><div  class='normal'>Almacen Origen</div></th>";
            html += "<th><div  class='normal'>Almacen Destino</div></th>";
            html += "</thead><tbody style='height:300px;'>";
            var listaProducto = window.parent.listaProducto;
            for (var i = 0; i < lista.length; i++) {
                var obj = lista[i];
                var origen = listaAlmacen[obj.almacenOrigen];
                var destino = listaAlmacen[obj.almacenDestino];
                var producto = listaProducto["p" + obj.producto_id];
                html += "<tr >";
                html += "<td><div style='width:80px'>" + obj.fecha + "</div></td>";
                html += "<td><div style='width:70px' class='subrayar'  onclick=\"redireccionar('Traspaso'," + obj.id_traspasoProducto + ")\">" + obj.nroDocumento + "</div></td>";
                ...
                html += "<td><div  class='normal'>" + origen.nombre + "</div></td>";
                html += "<td><div  class='normal'>" + destino.nombre + "</div></td>";
                html += "</tr>";
            }
            html += "</tbody>";
